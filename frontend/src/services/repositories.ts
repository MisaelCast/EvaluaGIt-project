...
export async function getProjectRepositories(projectId: string): Promise<RepositoryResponse[]> {
  const headers = await getAuthHeaders()

  if (!headers.Authorization) {
    throw new Error('Inicia sesión para ver los repositorios')
  }
...
export async function createRepository(data: RepositoryCreate): Promise<RepositoryResponse> {
  ...
  if (!headers.Authorization) {
    throw new Error('Inicia sesión para vincular repositorios')
  }
...
export async function deleteRepository(repositoryId: string): Promise<void> {
  ...
  if (!headers.Authorization) {
    throw new Error('Inicia sesión para eliminar repositorios')
  }
